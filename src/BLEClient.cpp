...
void BLEClient::gattClientEventHandler(
	...
	ESP_LOGI(LOG_TAG, "gattClientEventHandler [esp_gatt_if: %d] ... %s",
	...
	switch(event) {
		...
		case ESP_GATTC_DISCONNECT_EVT: {
			...
			m_semaphoreSearchCmplEvt.give(ESP_FAIL);
			if(!m_isConnected)
				break;
...
// TODO implement retrieving services from cache
	...
	if(isConnected()) {
		...
		ESP_LOGE(LOG_TAG, "Client Not Connected, unable to retrieve services");
